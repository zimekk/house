...
export function cross(a: Point, b: Point = a): Point {
  const [ax] = a;
  const [, by] = b;
...
export function crossx(a: Point, b: Point = a): Point {
  const [ax] = b;
  const [, by] = a;
...
export function flipx(a: Point): Point {
  const [ax, ay] = a;
...
export function flipxy(a: Point): Point {
...
const rooms2 = (
  [a1, b1, c1, d1]: Point[],
  { h10, h11 }: { h10?: Point; h11?: Point } = {},
  view = Views.T,
) => {
  return ((tl, tr, br, bl) => {
    return [
      (([a2, c2]) =>
        [Views.A, Views.D].includes(view)
          ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
          : rect(a2, c2))([
        bl.current(),
      ...
      (([a2, c2]) =>
        ...
        // </g> */}
        bl.current(),
        shift(bl.shift([2.1, -3.44]), [0, 0]),
      ]),

      (([a2, c2]) =>
        [Views.A].includes(view)
          ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
          : [Views.BB].includes(view)
            ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
            : // [Views.B].includes(view)?rect(cross(a2,shift(h11, [0,1])), cross(c2, shift(h10, [0,1])))
              //   .map(flipy):
              rect(a2, c2))([
        br.current(),
      ...
      (([a2, c2]) =>
        [Views.C].includes(view)
          ? rect(cross(a2, h11), cross(c2, h10)).map(flipxy)
          : [Views.D].includes(view)
            ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
            : rect(a2, c2))([
        tl.current(),
      ...
      (([a2, c2]) =>
        [Views.D].includes(view)
          ? rect(cross(a2, h11), cross(shift(c2, [-0.7, -1.94]), h10)).map(
              flipy,
            )
          : [Views.B].includes(view)
            ? rect(a2, shift(c2, [-0.7, -1.94])).map(flipy)
            : rect(a2, shift(c2, [-0.7, -1.94])))([
        //   <path
        //     d={
        //       (
        //         [Views.B].includes(view)?rect(shift(a2, [0, 1.5]), c2).map(flipy):
        //         rect(shift(a2, [0, 1.5]), c2)
      ...
      (([a2, c2]) =>
        rect(shift(a2, [-0.7, 0]), shift(c2, [0, -1.78])))([
        //   <path
      ...
      (([a2, c2]) =>
        [Views.C].includes(view)
          ? rect(cross(a2, h11), cross(c2, h10)).map(flipxy)
          : rect(a2, c2))([tl.current(), shift(tr.current(), [0, 3.6])]),

      (([a2, c2]) =>
        [Views.A].includes(view)
          ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
          : rect(a2, c2))([bl.current(), shift(br.current(), [0, 0])]),
    ];
  })(
...
(([b1,d1]) => (
    ...
    {((tl, tr, br, bl) => (
      <g>
        {[Views.A, Views.D, Views.T].includes(view) && (
          <g>
            {(([a2, c2]) => (
              <path
                d={([Views.A, Views.D].includes(view)
                  ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
                  : rect(a2, c2)
                )
                  .map((p, i) => (i > 0 ? line(p) : move(p)))
        ...
        {[Views.A, Views.T].includes(view) && (
          <g>
            {(([a2, c2]) => (
              <path
                d={([Views.A].includes(view)
                  ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
                  : rect(a2, c2)
                )
                  .map((p, i) => (i > 0 ? line(p) : move(p)))
            ...
            {(([a2, c2]) => (
              <g>
                ...
                <path
                  d={rect(
                    shift(cross(a2, c2), [-1.38 - 0.16, (2.64 - 0.16) / 2]),
                    shift(cross(a2, c2), [0, -(2.64 + 0.16) / 2]),
                  )
                    .map((p, i) => (i > 0 ? line(p) : move(p)))
                    .concat("z")
        ...
        {[Views.A, Views.B, Views.T].includes(view) && (
          <g>
            {(([a2, c2]) => (
              <path
                d={([Views.A].includes(view)
        ...
        {[Views.C, Views.D, Views.T].includes(view) && (
          <g>
            {(([a2, c2]) => (
              <path
                d={([Views.C].includes(view)
                  ? rect(cross(a2, h11), cross(c2, h10)).map(flipxy)
                  : [Views.D].includes(view)
                    ? rect(cross(a2, h11), cross(c2, h10)).map(flipy)
                    : rect(a2, c2)
        ...
        {[Views.B, Views.T].includes(view) && (
          <g>
            {(([a2, c2]) => (
              <g>
                <path
                  d={([Views.D].includes(view)
                    ? rect(
                        cross(a2, h11),
                        cross(shift(c2, [-0.7, -1.94]), h10),
                      ).map(flipy)
                    : [Views.B].includes(view)
                      ? rect(a2, shift(c2, [-0.7, -1.94])).map(flipy)
                      : rect(a2, shift(c2, [-0.7, -1.94]))
                ...
                />
                <path
                  d={([Views.B].includes(view)
                    ? rect(shift(a2, [0, 1.5]), c2).map(flipy)
                    : rect(shift(a2, [0, 1.5]), c2)
                  )
                    .map((p, i) => (i > 0 ? line(p) : move(p)))
                    .concat("z")
    ...
    <g transform={`scale(${25}) translate(${[14, 4].join(",")})`}>
      {(({
        ...
        <g>
          ...
          {/* {(rooms)(rect(cross(h, a), e))} */}
          {rooms2(rect(cross(h, a), e)).map((points, i) => (
            <path
              key={i}
              d={points
                .map((p, i) => (i > 0 ? line(p) : move(p)))
      ...
      {true &&
        (({
          ...
          /> */}
            ...
            {/* {(([a2, c2]) => (
                    <path
                      d={rect(cross(a2,h11), cross(c2, h10))
                        .map(flipy)
                        .map((p, i) => (i > 0 ? line(p) : move(p)))
                        .concat("z")
                        .join(" ")}
            ...
            {rooms2(rect(cross(h, a), e), { h10, h11 }, Views.A).map(
              (points, i) => (
                <path
                  key={i}
                  d={points
        ...
        />
            ...
            <text style={{ fontSize }} x={-12} y={1}>
              Side A
            </text>
            {Object.entries({ h_0, h00, h01, h10, h11 }).map(([l, p]) =>
              (([x, y]) => (
                <text key={l} style={{ fontSize }} x={x} y={y}>
                  {l}
                </text>
      ...
      {(({
        ...
        />
          ...
          {/* {(rooms)(rect(cross(h, a), e), {h10,h11}, Views.B)} */}
          ...
          {(({
            ...
            <g transform="translate(0,4)">
              {/* {(rooms)(rect(cross(h, a), e), {h10,h11}, Views.A)} */}
              {rooms2(rect(cross(h, a), e), { h10, h11 }, Views.B)
                .map((points) => points.map(([x, y]): Point => [-y, x]))
                .map((points, i) => (
                  <path
                    key={i}
                    d={points
      ...
      />
          ...
          /> */}
            ...
            {rooms(rect(cross(h, a), e), { h10, h11 }, Views.C)}

            {Object.entries({ h_0, h00, h01, h10, h11, d00, d01 }).map(
              ([l, p]) =>
                (([x, y]) => (
                  <text key={l} style={{ fontSize }} x={x} y={y}>
                    {l}
      ...
      {/* DY */}
      {false &&
        (({ a, e, f, h, i, j, k, l, m, h_0, h00, h01, h10, h11, d00, d01 }) => (
        ...
        />
            ...
            {(({
              ...
              <g transform="translate(10,0)">
                {rooms(rect(cross(h, a), e), { h10: h00, h11: h01 }, Views.D)}
              </g>
            ))(model.pointsA())}

            <text style={{ fontSize }} x={-5} y={0}>
              View D
            </text>
            {Object.entries({ h_0, h00, h01, h10, h11, d00, d01 }).map(
              ([l, p]) =>
